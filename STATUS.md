...
| **M7.6g–p** | Full PMMU: short + long form, multi-level walk, TT0/TT1, WP/S/IS/U/M, PTEST→MMUSR | 8/8 |
| **M7.6r** | 030 exception stack frame format (format-A 32-byte BERR, format-0 8-byte, variable-pop RTE) | 8/8 |
| **M7.6s** | SE/30 trace diagnostics (fault_addr/D7/A6 logging, 128 MB default RAM) | 8/8 |
| **M7.6t–y** | minivmac-reference fixes: SCC RR0 DCD cleared on SE/30, RTC PB0 idle HIGH, VIA1 PB3 by model | 8/8 |
| **M7.6x** | SRP/CRP canonical "MMU disabled" init (0x7FFF0001 from minivmac) | 8/8 |
| **M7.6aa–ab** | Snapshot model field round-trips through write/load | 8/8 |
| **M7.6ac** | SE/30 reset snapshot capability in trace tool (SE30_SAVE_RESET) | 8/8 |
| **M7.6ad** | JIT: terminate block at PMMU ops on SE/30 (helper-step bridge bug) | 8/8 |
| **M7.6ae** | JIT dispatcher: raise deferred BERR after each block (matches interp loop) | 8/8 |
| **M7.6af** | SE/30 lockstep test in ctest (diff_jit_se30_reset_lockstep) | 9/9 |
| **M7.6ag** | VIA1 ORB init differs by model (PB7 = vSndEnb on Plus, vSndOff on SE/30) | 9/9 |
| **M7.6ah** | SE/30 lockstep extended to 200K cycles (verified through 10M / 986K blocks) | 9/9 |
| **M7.6an** | JIT F-line dispatch: route PMMU ops to helper bridge before fline_trap | 9/9 |

**Current state:**
* **Interpreter**: All 68020/030 integer ISA implemented (M7.0). Full
  PMMU translation landed (M7.6a-p). 030 exception stack frame format
  for SE/30 (format-A BERR frame with fault address at SP+0x0E,
  format-0 for non-BERR, variable-pop RTE). Boots SE/30 ROM through
  47M instructions, stalls in post-PMMU hot loop at PC=0x40802EDC —
  the ROM polls SCC channel A RR0 for RX_AVAIL (Macsbug detection)
  with D7 bits 16/17/22 governing the flow. With SE30_PATCH_2C6C
  (NOPing the unconditional BRA to diagnostic mode at 0x40802C6C),
  the trace advances to a DIFFERENT hot loop at PC=0x408034A6.
* **JIT**: Hybrid termination (M7.0) + 14 native inline arms (M7.5).
  Three SE/30-specific bugs FIXED via diff-jit lockstep:
  - M7.6ae: JIT dispatcher missing deferred-BERR raise after blocks
    (now mirrors m68k_run_until).
  - M7.6ad → M7.6an: PMMU ops were being caught by the F-line trap
    arm and raised vec 11 (illegal-instr) instead of bridging to
    do_pmmu. Now correctly route to helper_step. PMMU inline restored.
  **JIT proven correct for SE/30 boot through 10M cycles / 986K
  blocks** via diff_jit_trace.
* **Tests**: **9/9 ctest cases green** throughout. New
  diff_jit_se30_reset_lockstep runs 200K cycles of JIT-vs-interp from
  SE/30 reset, snapshot generated by scripts/snap-se30-reset.sh
  (gitignored). Plus regression still bit-identical across every M7
  commit.

**Out of scope — still pending:**
* **M7.2** — vMac SE/30 lock-step harness. **EFFECTIVELY DONE** via
  diff_jit_se30_reset_lockstep — the spec-authoritative interp serves
  the same purpose as a third-party reference oracle. A true minivmac
  IIx variant remains TODO but is no longer load-bearing.
* **M7.3 (boot completion)** — Reach the floppy-question-mark / boot
  icon. Blocked on deeper hardware modeling: VIA1/VIA2 timer + IRQ
  delivery, full ASC FIFO state machine, ADB protocol. Requires
  multi-session work.
* **M7.4** — System 7.0 from InfiniteHD. Blocked on M7.3.
* **M7.6 (real PTW)** — **DONE** as M7.6g-p (multi-level walk, WP/S/IS
  enforcement, U/M maintenance, PTEST→MMUSR demux, format-A BERR
  frame with fault address).
