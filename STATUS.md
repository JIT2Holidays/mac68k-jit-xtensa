...
* **JIT**: Hybrid termination (M7.0) + 14 native inline arms (M7.5).
  Two SE/30-specific bugs FIXED via lockstep diff (M7.6ad/ae):
  PMMU helper-step bridge corrupted register file (now terminates
  block); JIT dispatcher missing deferred-BERR raise after blocks
  (now mirrors m68k_run_until). **JIT proven correct for SE/30 boot
  through 10M cycles / 986K blocks** via diff_jit_trace.
