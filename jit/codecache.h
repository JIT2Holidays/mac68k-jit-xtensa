...
/* Single-arena code cache. The host port allocates the backing memory with
   ...
   Three allocators, selected at codecache_init time via `mode`:

   - CC_MODE_BUMP (0, default): bump allocator. codecache_alloc fails
     (NULL) once the arena is full; the dispatcher then interp-falls-back
     for new PCs. A working set larger than the arena hits a hard
     performance cliff.

   - CC_MODE_LRU (1): free-list allocator with coldness/LRU eviction.
     When no free span fits, the dispatcher's evict callback drops the
     *coldest* cached block (lowest last-use cycle) and returns its
     span. Per-block 8-byte tag in m68k_block; an O(blocks) victim scan
     + a free-list walk per compile; suffers free-list fragmentation.

   - CC_MODE_FIFO (2): circular ring with FIFO eviction. The write
     cursor wraps; a new block overwrites — and so evicts — whichever
     blocks its bytes land on, always the oldest-compiled ones. No tag,
     no free list, no fragmentation; but it cannot protect a hot block
     that happens to sit under the cursor.

   The mode is runtime-selectable so a single binary can sweep through
...
typedef struct {
    ...
    u32  cap;
    u8   mode;            /* CC_MODE_* */
    /* LRU mode only: intrusive free spans, 8-byte {size,next} header at
       the start of each free span; address-sorted. */
    u32  free_head;       /* offset of first free span, or CC_NIL */
    cc_evict_fn       evict;        /* LRU mode */
    cc_evict_range_fn evict_range;  /* FIFO mode */
    void *evict_ctx;
} codecache;
...
/* LRU mode: return a [offset, offset+size) span to the free list
   (coalescing). No-op in bump and FIFO modes. */
